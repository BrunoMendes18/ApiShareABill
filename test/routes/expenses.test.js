...
const despA = { id:10000, nome: 'Despesa1', quanti: 50, tipo: 1, pago: 10000 };
const grupo = { id: 10001, nome: 'Grupo2', desc: 'descricao' , data: new Date(), admin: 10001 };
...
test('Test #2 - Inserir despesa', () => {
    ...
    .set('authorization', `bearer ${userA.token}`)
      .send({ nome: 'Xavier22',quanti:10,tipo:1,pago:userA.id })
      .then((res) => {
...
describe('Validação inserir despesa', ()=>{
    const testTemplate = (newData, errorMessage) => {
        return request(app).post(MAIN_ROUTE)
        .set('authorization', `bearer ${userA.token}`)
        .send({nome: 'Xavier25',quanti:10,tipo:1,grupo_id: 1,pago:userA.id, ...newData})
        .then((res)=>{
...
test('Test #3 - Atualizar despesa',()=>{
    ...
    .insert({nome: 'Atualizar Despesa Teste',quanti:69,tipo:1,grupo_id: grupo.id ,pago:userA.id},['id'])
...
test('Test #4 - Ver despesa por id', () => {
   return app.db('despesa')
    .insert({nome:'Ver despesa por id',quanti:79,tipo:1,grupo_id: grupo.id,pago:userA.id},['id'])
...
test('Test #6 - Eleminar despesa', () => {
    return app.db('despesa')
    .insert({nome:'Remover despesa por id',quanti:79,tipo:1,grupo_id: null,pago:userA.id},['id'])
